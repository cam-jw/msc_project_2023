...
from seg_models.sam_multi_swag.segment_anything import sam_model_registry
from seg_models.sam_multi_swag.swag_utils.swag import SWAG
...
class Sam_Multi_Swag(Sam_Super):
    def __init__(
            self,   
            seed: int,
            max_num_models: int,
        ...
        self.swag_models = [
            SWAG(base=decoder.to("cpu"), max_num_models=max_num_models) for decoder in self.model.mask_decoder.output_hypernetworks_mlps
            ]
        ...
        # self.sampling_decoder = self.swag_models.base
            ...
            for (image, label, axis) in train_loader:
                ...
                if self.do_swag or self.step > self.lr_schedule.swag_start:
                    ...
                    if self.step % self.lr_schedule.swag_freq  == 0:
                        for i in range(len(self.swag_models)):
                            # print("collating swag", i)
                            self.swag_models[i].collect_model(
                                self.model.mask_decoder.output_hypernetworks_mlps[i].to("cpu")
                            )
                            self.model.mask_decoder.output_hypernetworks_mlps[i].to(self.device)
    ...
    def sample(
        ...
        if self.validating:
            ...
            self.multimask_output = True
            for i in range(len(self.swag_models)):
                self.swag_models[i].sample(scale=1., device=self.device)
                self.model.mask_decoder.output_hypernetworks_mlps[i] = self.swag_models[i].base.to(self.device)
            ...
            prediction = torch.swapaxes(prediction, 0, 1)[:, None, :, :, :]
            for i in range(len(self.swag_models)):
                self.swag_models[i].base.to("cpu")
            self.multimask_output = False
            return prediction
